...
const Navbar = () => {
  ...
  const handleScroll = () => {
    // Adjust threshold as you like (in pixels)
  ...
  // Home: always white navbar
  // About: always bg-zinc-950 (dark)
  // Other pages: gradient at top, white when scrolled
  const navBgClass = isAbout
    ? "bg-zinc-950 text-white shadow-md backdrop-blur"
    : isHome
    ? "bg-white/95 text-black shadow-md backdrop-blur"
    : isScrolled
    ? "bg-white/95 text-black shadow-md backdrop-blur"
    : "bg-gradient-to-r from-pink-500 to-red-600 text-white shadow-none";

  // Determine if background is dark → then use white text, else black text
  const isDarkBackground = isAbout || (!isHome && !isScrolled);

  const logoTextClass = isDarkBackground ? "text-white" : "text-black";
  const iconColorClass = isDarkBackground ? "text-white" : "text-black";
  const mobileToggleColorClass = isDarkBackground ? "text-white" : "text-black";
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-2 flex justify-between items-center">
        ...
        <Link to="/" className="flex items-center">
          <div className="text-left leading-tight">
            <div className="text-xl sm:text-2xl font-extrabold tracking-[0.18em]">
              <span className={logoTextClass}>PRINT</span>
            </div>
            <div className="text-sm sm:text-base font-semibold tracking-[0.25em]">
              <span className={logoTextClass}>E</span>
              <span className="text-red-600">X</span>
              <span className={logoTextClass}>P</span>
              <span className={logoTextClass}>R</span>
              <span className={logoTextClass}>E</span>
              <span className={logoTextClass}>S</span>
              <span className={logoTextClass}>S</span>
            </div>
          </div>
        </Link>
        ...
        <div className="hidden md:flex md:items-center space-x-6">
          ...
          <div className="flex justify-center md:block mt-0">
            <Link
              className={`relative transition-colors duration-300 transform hover:opacity-80 ${iconColorClass}`}
              to="#"
            >
              <svg
                className="w-5 h-5"
        ...
        <div className="flex md:hidden">
          <button
            ...
            type="button"
            className={`focus:outline-none ${mobileToggleColorClass}`}
